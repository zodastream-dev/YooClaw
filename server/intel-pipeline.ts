...
// 注意：保留概念词，不含过于宽泛的词（如纯"利率""贷款"太泛）
const BANKING_TOPIC_TERMS = [
  // 货币政策（核心）
  '准备金', 'LPR', 'MLF', 'SLF', '逆回购', '公开市场', '降准', '降息', '货币政',
  // 信贷市场
  '社融', '信贷投放', '存贷款', '净息差', '对公业务', '普惠金融', '零售转型',
  // 风险监管（含金监总局/央行/银保监发布的新规）
  '资本充足', '拨备覆盖', '不良贷款', '反洗钱', '监管罚单', '监管评级',
  '金监总局', '央行发布', '央行开展', '银保监', '金融监管',
  // 地方政府/基建
  '地方债', '专项债', '城投', '化债', '隐性债', '重大项目',
  // 房地产
  '房地产贷款', '房贷', '房市', '楼市',
  // 金融/财政
  '金融支持', '金融服务', '财政金融', '金融科技', '绿色金融',
  // 宏观经济
  'GDP', 'CPI', 'PPI', 'PMI', '国民经济', '经济数据', '经济运行',
  '规模以上工业', '固定资产投资', '社会消费品', '进出口',
  '人民币汇率', '外汇储备', '债券发行', '发行债券',
  // 企业融资
  'IPO', '发债', '增发', '信用债', '融资需求',
  // 银行竞争（具体银行名）
...
export async function callIntel(effectiveKwArr: string[], src: any, objectName?: string, allSources?: any[]): Promise<any[]> {
  ...
  // V2.7: Include Serper whitelist results in URL whitelist too
