...
var PROVIDER_NAMES={metaso:'秘塔',serper:'Serper',newsbank:'Serper新闻库',xiaohongshu:'小红书',zhihu:'知乎',weibo:'微博',wechat:'微信','multi-engine':'多引擎',tavily:'Tavily','tianapi-generalnews':'天聚综合','tianapi-keji':'天聚科技','tianapi-ai':'天聚AI','tianapi-guonei':'天聚国内','tianapi-world':'天聚国际','tianapi-social':'天聚社会','tianapi-caijing':'天聚财经','tianapi-internet':'天聚互联网'};
...
function renderSourceForm(wi,si){
  ...
  s+='</div>';
  s+='<div class="kw-add-row"><input class="kw-add-input" id="objInput_'+wi+'_'+si+'" placeholder="输入对象名称后回车添加..." onkeydown="if(event.key===\\'Enter\\'){event.preventDefault();addObject('+wi+','+si+')}" readonly onfocus="this.removeAttribute(\'readonly\')" autocomplete="off" name="obj'+wi+'_'+si+'">';
  ...
  s+='</div>';
  s+='<div class="kw-add-row"><input class="kw-add-input" id="kwInput_'+wi+'_'+si+'" placeholder="输入关键词后回车添加..." onkeydown="if(event.key===\\'Enter\\'){event.preventDefault();addKeyword('+wi+','+si+')}" readonly onfocus="this.removeAttribute(\'readonly\')" autocomplete="off" name="kw'+wi+'_'+si+'">';
